...
- **Face Recognition Engine:** Built on the open-source `face_recognition` library, which wraps dlib's C++ ResNet-34 deep learning model.
...
- **Verification Pipeline:**
  ...
  4. A strict tolerance threshold of 0.45 is enforced, requiring the match to fall within this distance to be verified.
- **Anti-Spoofing & Contextual Security Layer:** To prevent students from marking attendance remotely, SaveState uses a dynamic, time-limited room code system:
  ...
  - The backend cross-verifies that the matched face corresponds exactly to the credentials of the logged-in student account.
...
- Replaced manual/proxy-prone attendance with a two-factor verification model: biometric (face match) and contextual (time-limited room code).
- Demonstrates the ability to design cost-conscious ML infrastructure by running a real facial recognition pipeline on budget hosting through frame downscaling and manual memory management.
...
### Core Problem Solved
Managing daily digital operations has become fragmented across subscriptions, appointments, deadlines, emails, and documents scattered across disconnected tools. OttoAi consolidates this into an intelligent AI-powered operations agent that monitors priorities, summarizes workflows, and lets users execute actions directly through AI orchestration.
...
- **Ecosystem Integrations & OAuth Pipelines:** 4+ Google services integrated with robust token management:
  - **Gmail:** AI-generated inbox summaries and AI-drafted replies generated directly in-app.
  - **Google Docs & Drive:** documents are analyzed and summarized by LLMs so users do not need to open and parse each file manually.
...
- **Proctoring AI Detection Features:**
  ...
  - Continuous face-activity and presence tracking.
...
- **Gesture Mapping Logic:**
  ...
  |---|---|
  | Right hand raised or extended | Punch |
  | Left hand raised or extended | Kick |
...
- Core strengths centered around Applied AI/ML, LLM Orchestration & Automation, Computer Vision, and Backend Architecture.
